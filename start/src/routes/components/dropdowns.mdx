...
<GithubSource name="components/dropdowns.mdx" />
# Dropdowns
...
<div class="row gy-5 pt-3">
    ...
    <Example.Preview>
      ...
      <div class="mt-4 mb-3">
        Since the above is such a common configuration solid-bootstrap provides
    ...
    <Example.Preview>
      <>
        ...
        <Stack direction="horizontal" gap={2} class="mt-2">
          {[DropdownButton, SplitButton].map((DropdownType, idx) => (
            <DropdownType
              as={ButtonGroup}
              id={`dropdown-button-drop-${idx}`}
